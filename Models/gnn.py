...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_classes, num_tasks, num_layer=5, emb_dim=300,
                 ...
                 num_mlp_layers=1, sequential_k_wl=False, k_wl=0
                 ):
        '''
        ...
        if virtual_node:
            ...
            self.gnn_node = GNN_node(num_layer, emb_dim, JK=JK, drop_ratio=drop_ratio, residual=residual,
                                     gnn_type=gnn_type, node_encoder=node_encoder, edge_encoder=edge_encoder,
                                     k_wl=self.k_wl, sequential_k_wl=self.sequential_k_wl)
    ...
    def set_mlp(self, graph_features=0, copy_emb_weights=False):
        ...
        for i in range(self.num_mlp_layers):
            in_size = hidden_size if i > 0 else in_emb + graph_features
